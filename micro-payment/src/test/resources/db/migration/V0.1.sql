CREATE TABLE `payment_main` (
    ...
    `user_id`         VARCHAR(50)    NOT NULL COMMENT '支付用户ID',
    `amount`          DECIMAL(12, 2) NOT NULL COMMENT '支付金额',
    `status`          TINYINT        NOT NULL COMMENT '支付状态 1 待支付 2 已支付 3 已取消 4 已退款 5 已失败',
    `payment_time`    DATETIME       NULL COMMENT '支付时间',
    `payment_method`  TINYINT        NOT NULL COMMENT '支付平台 1 余额 2 微信 3 支付宝',
    `payment_channel` VARCHAR(50)    NOT NULL COMMENT '支付渠道 支付宝当面付 微信JSAPI等',
    PRIMARY KEY (`id`),
...
CREATE TABLE `payment_refund` (
    ...
    `payment_id`        INT            NOT NULL COMMENT '退款的支付ID',
    `payment_no`        VARCHAR(50)    NOT NULL COMMENT '退款的支付编号',
