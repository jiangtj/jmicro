...
项目使用 [vanniktech/gradle-maven-publish-plugin](https://github.com/vanniktech/gradle-maven-publish-plugin) 发布所有 `micro-*` 库模块（含 `micro-dependencies` BOM）。`demo-*` 为示例应用，不参与发布。

### 前置条件

1. 注册 [Central Portal](https://central.sonatype.org/register/central-portal/) 账号并完成命名空间 `com.jiangtj.micro` 的验证。
2. 准备一对用于签名制品的 GPG 密钥，并分发公钥到公钥服务器。

### 配置凭据与签名密钥

凭据与密钥**不要写入仓库**。请放入用户级 `~/.gradle/gradle.properties`（或对应的环境变量）：

```properties
# Central Portal 生成的 user token（非登录密码）
...
发布目标、签名开关已在根 `gradle.properties` 中开启（`mavenCentralPublishing`、`signAllPublications`、`mavenCentralAutomaticPublishing`）。

### 发布命令

```shell
# 快照版本（版本号以 -SNAPSHOT 结尾）
./gradlew publishToMavenCentral

# 正式版本：上传后自动从 Central Portal 发布
./gradlew publishAndReleaseToMavenCentral
```

若关闭了 `mavenCentralAutomaticPublishing`，上传后需手动前往 Central Portal 点击 Publish 完成发布。

> 发布前可用 `./gradlew :micro-common:publishToMavenLocal` 在本地 Maven 仓库验证产物与 POM。
