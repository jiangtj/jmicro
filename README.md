...
在开发阶段，建议使用本地安装。即 `clone` 后，`./gradlew publishToMavenLocal` 将各模块发布到本地 Maven 仓库（需 `maven-publish` 插件，已在所有 `micro-*` 模块中配置；`demo-*` 应用不在发布范围内），当然也可直接引用 Maven Central 上已发布的 lib
