...
- `getAllConfig()` 返回排序后的配置视图（按分组 `group.order` 与 `order`），并对 `secret=true` 或 `bcrypt=true` 的项做脱敏（`******`），对带 `formatter` 的项生成 `formatedValue`。
- **bcrypt 支持**：配置项可设置 `bcrypt=true`（通常配合 `secret=true` 用于密码类项）。通过 `updateConfig(key, value)` 写入明文时，`SystemConfigService` 会先使用 `BCryptUtils`（`org.springframework.security.crypto.bcrypt.BCryptPasswordEncoder`）对明文进行哈希后再持久化；若传入值已是 bcrypt 哈希（以 `$2a$`/`$2b$`/`$2y$` 开头）则跳过二次哈希。读取时统一以 `******` 脱敏展示，校验明文可用 `BCryptUtils.matches(raw, stored)`。
